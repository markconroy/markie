 ...
 */
#[AiApiExplorer(
  id: 'object_detection_generator',
  title: new TranslatableMarkup('Object Detection Explorer'),
  description: new TranslatableMarkup('Contains a form where you can experiment and test the AI object detection.'),
)]
final class ObjectDetectionGenerator extends AiApiExplorerPluginBase {
  ...
  public function isActive(): bool {
    return $this->providerManager->hasProvidersForOperationType('object_detection');
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state): array {
    ...
    if ($request->query->get('provider_id')) {
      $form_state->setValue('object_detect_ai_provider', $request->query->get('provider_id'));
    }
    if ($request->query->get('model_id')) {
      $form_state->setValue('object_detect_ai_model', $request->query->get('model_id'));
    }

    $form = $this->getFormTemplate($form, 'object-detect-response');

    $form['left']['image'] = [
      '#type' => 'file',
      '#accept' => '.jpg, .jpeg, .png',
      '#title' => $this->t('Upload your image here. When submitted, your provider will detect objects. Please note that each query counts against your API usage if your provider is a paid provider.'),
    ...
    $this->aiProviderHelper->generateAiProvidersForm($form['left'], $form_state, 'object_detection', 'object_detect', AiProviderFormHelper::FORM_CONFIGURATION_FULL);
    $form['left']['object_detect_ai_provider']['#ajax']['callback'] = $this::class . '::loadModelsAjaxCallback';

    $form['left']['submit'] = [
      '#type' => 'submit',
      '#value' => $this->t('Detect Objects'),
      '#ajax' => [
        'callback' => $this->getAjaxResponseId(),
        'wrapper' => 'object-detect-response',
      ],
    ];
  ...
  public function getResponse(array &$form, FormStateInterface $form_state): array {
    try {
      $provider = $this->aiProviderHelper->generateAiProviderFromFormSubmit($form, $form_state, 'object_detection', 'object_detect');
      ...
      if (!$image_file) {
        $form['right']['response']['#context']['ai_response'] = [
          ...
          'message' => [
            '#type' => 'html_tag',
            '#tag' => 'div',
            '#value' => $this->t('Please upload an image file (JPG, JPEG, or PNG) to analyze.'),
            '#attributes' => [
      ...
      $input = new ObjectDetectionInput($image_file);
      $classification = $provider->objectDetection($input, $form_state->getValue('object_detect_ai_model'), ['ai_api_explorer'])
        ->getNormalized();
      ...
      if ($classification) {
        $form['right']['response']['#context']['ai_response']['table'] = [
          ...
          '#rows' => [],
          '#empty' => $this->t('There was an issue detecting objects.'),
        ];
        foreach ($classification as $row) {
          $form['right']['response']['#context']['ai_response']['table']['#rows'][] = [
            ...
            $this->t('<em>[:box]</em>', [
              ':box' => implode(', ', $row->getBox()),
            ]),
          ];
        }

        $form['right']['response']['#context']['ai_response']['code'] = $this->normalizeCodeExample($provider, $form_state, $image_file->getFilename());
      }
      else {
        $form['right']['response']['#context']['ai_response'] = [
          'heading' => [
            '#type' => 'html_tag',
            '#tag' => 'h3',
            '#value' => $this->t('No objects detected'),
          ],
          'message' => [
            '#type' => 'html_tag',
            '#tag' => 'div',
            '#value' => $this->t('The provider did not detect any objects. Please check your image and try again.'),
            '#attributes' => [
              'class' => ['ai-text-response', 'ai-error-message'],
            ],
          ],
  ...
  public function normalizeCodeExample(AiProviderInterface|ProviderProxy $provider, FormStateInterface $form_state, string $filename): array {
    ...
    $code['code']['#value'] .= "\$ai_provider = \Drupal::service('ai.provider')->createInstance('" . $form_state->getValue('object_detect_ai_provider') . '\');<br>';
    if (count($provider->getConfiguration())) {
    ...
    $code['code']['#value'] .= "\$input = new \Drupal\ai\OperationType\ObjectDetection\ObjectDetectionInput(\$image);<br><br>";
    $code['code']['#value'] .= "// Run the object detection.<br>";
    $code['code']['#value'] .= "\$response = \$ai_provider->objectDetection(\$input, '" . $form_state->getValue('object_detect_ai_model') . '\', ["your_module_name"]);<br><br>';
    $code['code']['#value'] .= "// Output is an array of detected objects.<br>";
    $code['code']['#value'] .= "\$detected_objects = \$response->getNormalized();<br>";
